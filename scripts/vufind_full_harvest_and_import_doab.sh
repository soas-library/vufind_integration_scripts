#!/bin/bash -v
# @name: vufind_full_harvest_and_import_doab.sh
# @version: 1.0
...
#
# @purpose:
# This script harvests and then imports records from Directory of Open Access Books into VuFind
...
find  /usr/local/vufind/local/harvest/DOAB/ -name '*.xml' -exec rm {} \;
find  /usr/local/vufind/local/harvest/DOAB/processed/ -name '*.xml' -exec rm {} \;
> /usr/local/vufind/local/harvest/DOAB/info.txt
rm /usr/local/vufind/local/harvest/DOAB/last_harvest.txt
rm /usr/local/vufind/local/harvest/DOAB/last_state.txt
